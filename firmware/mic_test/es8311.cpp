...
// Ported from the legacy i2c_master_* API to Wire: on arduino-esp32 3.3.x
// Wire owns the bus via the new (driver_ng) I2C driver, and mixing the two
// drivers aborts at boot ("CONFLICT! driver_ng is not allowed to be used
// with this old driver"). Wire is initialized by the sketch on SDA16/SCL15.
...
esp_err_t es8311_codec_init(void)
{
    ...
    // Freenove ships this commented out; without it capture sits at ~-40dBFS
    // (RMS ~40/32768, speech invisible). 30dB puts voice at conversational
    // distance well above the floor without clipping.
    ESP_RETURN_ON_ERROR(es8311_microphone_gain_set(es_handle, ES8311_MIC_GAIN_30DB), TAG, "set es8311 microphone gain failed");
    return ESP_OK;
}
